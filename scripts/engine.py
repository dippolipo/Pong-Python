...
def pygame_init(name, icon, screen_size): # TODO
    ...
    debug_screen = Surface(screen_size).convert_alpha()
    debug_screen.fill(pg.Color(0, 0, 0, 0))
...
class Collision:
    ...
    def two_segment_distance(a, b, c, d): # distance from A to CD segment
        ...
        if Collision.two_segment(a, b, c, d):
            seg = d - c
            return vec * ((c.x - a.x - (c.y - a.y) * (seg.x/seg.y))/(vec.x - vec.y * (seg.x/seg.y)))
        else:
            return vec
...
class RectCollision:
    @staticmethod
    def inside(pos, rect):
        if pos.x < rect.right and pos.x > rect.left and pos.y < rect.bottom and pos.y > rect.top:
            return True
        else:
            return False

    @staticmethod
    def ray(start_pos, end_pos, rect):
        dist = end_pos - start_pos
        if RectCollision.inside(start_pos, rect):
            if RectCollision.inside(end_pos, rect):
                return dist
            else:
                return RectCollision.ray(end_pos, start_pos, rect)
        else:
            if start_pos.y > rect.bottom:
                dist = Collision.two_segment_distance(start_pos, end_pos, rect.bottomright, rect.bottomleft)
            if start_pos.y < rect.top:
                dist = Collision.two_segment_distance(start_pos, end_pos, rect.topright, rect.topleft)
            if start_pos.x > rect.right:
                dist = Collision.two_segment_distance(start_pos, end_pos, rect.bottomright, rect.topright)
            if start_pos.x < rect.left:
                dist = Collision.two_segment_distance(start_pos, end_pos, rect.bottomleft, rect.topleft)
